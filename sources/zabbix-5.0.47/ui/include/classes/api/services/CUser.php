...
class CUser extends CApiService {
	 ...
	 *
	 * LDAP authentication support (CFrontendSetup::checkPhpLdapModule() and CLdapAuthValidator) was removed from
	 * this build [FR2.1][BR2.1][BR2.2]. A user group can still be configured with GROUP_GUI_ACCESS_LDAP in
	 * existing data, so login() can still route here; fail closed with the same message previously shown when
	 * the PHP LDAP extension was missing, instead of leaving a reference to the removed classes.
	 *
	 * @param array $user
	 *
	 * @return bool
	...
	protected function ldapLogin(array $user) {
		self::exception(ZBX_API_ERROR_PARAMETERS, _('PHP LDAP extension missing.'));
	}
