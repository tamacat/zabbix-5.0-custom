...
function zbx_setcookie($name, $value, $time = null) {
	// build-and-test: PHP8's setcookie() types $domain as non-nullable string (default ''); this used
	// to default to null, which is now deprecated. '' is the correct "no specific domain" value.
	setcookie($name, $value, isset($time) ? $time : 0, CSession::getDefaultCookiePath(), '', HTTPS, true);
	$_COOKIE[$name] = $value;
}

function zbx_unsetcookie($name) {
	// build-and-test: PHP8's setcookie() types $value as non-nullable string (default ''); this used
	// to pass null (via zbx_setcookie()), which is now deprecated. '' is the correct "clear" value.
	zbx_setcookie($name, '', -99999);
	unset($_COOKIE[$name]);
}
